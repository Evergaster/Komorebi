...
# PID y log del proceso de fondo
...
class WallpaperEngine:
    ...
    def play(self, video_path, screen_index=0, pause_on_max=False, volume=0, paused=False):
        ...
        # Generar thumbnail y establecer como fondo de GNOME (solo si es pantalla 0 por ahora)
        if screen_index == 0:
    ...
    def _send_quit_command(self):
        # Intentar detener vía socket primero
        cmd = self._background_player_base_cmd() + [
            "",  # Dummy path
            "--quit-service",
        ]
        ...
        # Forzar cierre de cualquier proceso remanente (zombies o instancias múltiples por race condition)
        try:
    ...
    def _set_gnome_background(self, video_path):
        # (Opcional) Extraer frame y ponerlo de fondo estático para transiciones suaves
        pass
    ...
    def get_thumbnail_path(self, video_path):
        ...
        # Hash del path para nombre único
    ...
    def get_thumbnail(self, video_path):
        ...
        try:
            # Extraer frame al 10% o 5 segundos
            subprocess.run([
